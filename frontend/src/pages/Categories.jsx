...
const Categories = () => {
  ...
  const getFilteredCategories = () => {
    ...
    return categories.filter((cat) => cat.type.toLowerCase() === filter.toLowerCase());
  };

  const filteredCategories = getFilteredCategories();

  const incomeCount = categories.filter((c) => c.type.toLowerCase() === "income").length;
  const expenseCount = categories.filter((c) => c.type.toLowerCase() === "expense").length;
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
            ...
            >
              ...
              {category.budget && (
                <div className="bg-white/5 rounded-lg p-3 mb-3">
                  <p className="text-gray-400 text-xs mb-1">Monthly Budget</p>
              ...
              {category.parentCategory && (
                <div className="pt-3 border-t border-gray-600/30">
                  <p className="text-gray-400 text-xs">
                    Parent: <span className="text-gray-300 font-medium">{category.parentCategory}</span>
                  </p>
                </div>
              )}
            </div>
        ...
        {filteredCategories.length === 0 && !loading && (
          <div className="bg-gradient-to-br from-gray-800/50 to-gray-700/50 border border-gray-600/50 rounded-2xl p-16 text-center backdrop-blur-sm">
            ...
            <h3 className="text-2xl font-bold text-white mb-3">No Categories Yet</h3>
