...
const EditAccount = () => {
  ...
  useEffect(() => {
    const loadAccount = async () => {
      try {
        ...
        setAccountData({ name: acc.name, balance: acc.balance });
      } catch (error) {
  ...
  const handleChange = (e) => {
    ...
    setAccountData((prev) => ({ ...prev, [name]: name === "balance" ? Number(value) : value }));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!accountData.name || accountData.balance < 0) {
